...
public class ResponseDemo extends HttpServlet {
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if ("yzm".equalsIgnoreCase(type)){
            demo9(request, response);
        }else if("yzm2".equalsIgnoreCase(type)){
            yzm2(request, response);
        }else if("xz".equalsIgnoreCase(type)){
            demo8(request, response);
    ...
    public void yzm2(HttpServletRequest request, HttpServletResponse response)throws ServletException, IOException{
        int width = 120;
        int height = 30;
    ...
    //验证码
    public void demo9(HttpServletRequest request, HttpServletResponse response)throws ServletException, IOException{
        //在内存中生成一张图片，随机生成字母或数字，文字写到图片上，输出图片
        int width = 120;//图片宽度
        int height = 30;//图片高度
        BufferedImage bufferedImage = new BufferedImage(width,height,BufferedImage.TYPE_INT_RGB);//图片
        Graphics2D graphics2D = (Graphics2D) bufferedImage.getGraphics();//画笔
        graphics2D.setColor(Color.YELLOW);//设置颜色
        graphics2D.fillRect(0, 0, width, height);//fillRect() x y 代表 左上顶点坐标  宽度 和高度
        //绘制边框
        ...
        //绘制干扰线
        int x1,y1,x2,y2;//x1 y1 代表第一个点坐标  x2 y2 第二个点
    ...
    //文件下载
    public void demo8(HttpServletRequest request, HttpServletResponse response)throws ServletException, IOException{

        String filePath_Dir = "D:\\work3\\dream-160118\\src\\main\\webapp\\files";
        String fileName = request.getParameter("name");
        String filePath = filePath_Dir+"\\"+fileName;

        request.setCharacterEncoding("UTF8");
        response.setContentType("text/html;charset=UTF8");
        response.setHeader("Content-Disposition","attchment;filename="+URLEncoder.encode(fileName,"UTF8"));
        ...
        response.sendRedirect("/page/download.jsp");
    }
    ...
    public void demo6(HttpServletRequest request, HttpServletResponse response)throws ServletException, IOException{
        ...
        response.getWriter().write("今天是星期六");
    }

    //使用字节流来返回数据
    public void demo5(HttpServletRequest request, HttpServletResponse response)throws ServletException, IOException{
        response.setHeader("Content-Type","text/html;charset=utf8");
        response.getOutputStream().write("今天是星期五".getBytes("UTF-8"));
    }
    ...
    public void demo4(HttpServletRequest request, HttpServletResponse response)throws ServletException, IOException{
        ...
        //页面输出时间
        response.getWriter().write("<h1>" + new Date().toString() + "</h1>");
    }

    //定时刷新
    public void demo3(HttpServletRequest request, HttpServletResponse response)throws ServletException, IOException{
        response.setContentType("text/html;charset=utf8");
        response.getWriter().write("页面5s后跳转");
        response.setHeader("Refresh","5;url=/response/login.jsp");

        //另一种方法是页面标签<meta http-equiv="Refresh" content="5;url=/response/login.jsp">参看response/refresh.jsp
    }
...
//            response.setStatus(302);//登陆失败使用重定向
//            response.setHeader("Location","/response/login.jsp");

            //存入一个值
            request.setAttribute("msg", "用户名或密码错误!!");
            //转发login.jsp
            request.getRequestDispatcher("/response/login.jsp").forward(request, response);
        }
    }
